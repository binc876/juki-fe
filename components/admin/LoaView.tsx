    ...
    <div className="space-y-6">
       ...
       <div className="bg-white rounded-[32px] p-10 shadow-sm mt-4">
          ...
          <div className="flex justify-between items-center mt-8 pt-4 border-t border-gray-100 text-sm text-gray-600">
            <div>
              Menampilkan <span className="font-bold">{meta.total > 0 ? (meta.page - 1) * meta.limit + 1 : 0}-{Math.min(meta.page * meta.limit, meta.total)}</span> dari <span className="font-bold">{meta.total}</span> Data
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => setMeta({...meta, page: Math.max(1, meta.page - 1)})} disabled={meta.page === 1} className="p-2 border rounded-lg hover:bg-gray-50 disabled:opacity-50"><ChevronLeft className="w-4 h-4" /></button>
              <span className="font-bold px-2">Halaman {meta.page} dari {meta.totalPage}</span>
              <button onClick={() => setMeta({...meta, page: Math.min(meta.totalPage, meta.page + 1)})} disabled={meta.page === meta.totalPage} className="p-2 border rounded-lg hover:bg-gray-50 disabled:opacity-50"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
       </div>
